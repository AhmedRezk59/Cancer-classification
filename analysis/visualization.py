...
def plot_count(dataframe: pd.DataFrame, variables:list,hue:str,
               data_type:DataType = DataType.FIGURES) -> None:
    """
    Plot the count of unique values in a specified column of a dataframe.

    Args:
        dataframe (pd.DataFrame): _description_
        variables (list): _description_
        hue (_type_, optional): _description_. Defaults to DataType.FIGURES.
    """
    plt.figure(figsize=FIG_SIZE)
    plt.suptitle("Count of unique values in column")
    plot_iterator:int = 1
    for i in variables:
